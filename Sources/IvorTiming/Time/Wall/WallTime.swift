...
extension WallTime: TimeProtocol {
    ...
    public func duration(to time: Self) -> (duration: WallDuration,
                                            direction: DurationDirection)? {
        let val1 = numberValue
        let val2 = time.numberValue

        if val1 < val2 {
            return (WallDuration(val2 - val1), .forward)
        }

        if val1 > val2 {
            return (WallDuration(val1 - val2), .backward)
        }

        return (.zero, .same)
    }

    public func moved(by duration: WallDuration,
